...
Proof.
  intro l.
  induction l0 as [ | l0' Hl0'].
  - reflexivity.
  - simpl. rewrite -> IHHl0'. reflexivity.
Qed. 
...
Proof.
  ...
  - simpl. rewrite -> IHHl2'. rewrite -> app_assoc.
    reflexivity.
Qed.      
...
Proof.
  ...
  induction l0 as [ | l0' Hl0'].
  - simpl. reflexivity.
  - simpl. rewrite -> rev_app_distr. simpl. rewrite -> IHHl0'. 
    reflexivity.
Qed.
...
Fixpoint split {X Y : Type} (l : list (X*Y))
  ...
  match l with
  | [] => ([],[])
  | (h1, h2) :: tail => (h1 :: (fst (split tail)), h2 :: (snd (split tail)))
  end. 
...
Theorem map_rev_aug1: forall (X Y: Type) (f : X -> Y) (l1 l2 : list X),
  map f (l1 ++ l2) = map f (l1) ++ map f (l2).
Proof.
  intros.
  induction l1 as [ | l1' Hl1'].
  - simpl. reflexivity.
...
Proof.
  ...
  induction l as [ | l' Hl'].
  - simpl. reflexivity.
  - simpl. rewrite -> map_rev_aug1. rewrite -> IHHl'. simpl.
    reflexivity.
Qed. 
...
(* 
   change a list of integer to a string e.g.
   ['2';'0';'1';'9'] ['']  
*)
...
Example test_fold_length1 : fold_length [4;7;1] = 3.
Proof.
simpl.
reflexivity.
Qed.
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y :=
  fold (fun x list_of_y => f x :: list_of_y) l [].
...
Theorem fold_map_corret: forall (X Y: Type) (f: X->Y) (l : list X),
