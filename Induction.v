...
Proof.
  ...
  induction m as [ | m' Hm'].
  - reflexivity.
  - simpl. rewrite -> Hm'. rewrite -> plus_swap. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite -> mult_n_Sm. rewrite Hn'. reflexivity.
Qed. 
...
(** **** Exercise: 3 stars, standard, optional (more_exercises)  
    ...
    down your prediction.  Then fill in the proof.  (There uyhis no need
...
Proof.
  ...
  induction n as [ | n' Hn'].
  - simpl. reflexivity.
  - simpl. rewrite -> mult_plus_distr_r. rewrite -> Hn'.
    reflexivity.
Qed.
...
Fixpoint bin_to_nat (m:bin) : nat :=
  ...
  | B m' => match m' with
            | Z => S 0
            | _ => S (bin_to_nat m' + bin_to_nat m')
            end
  end.

Theorem bin_to_nat_1: forall n: bin,
  bin_to_nat(incr (A n)) = bin_to_nat(incr n) + bin_to_nat(n).
Proof.
Admitted.
...
Proof.
  intro n.
  induction n as [ | n' | n'' Hn ].
  - simpl. reflexivity.
  - assert (bin_to_nat(incr (A n')) = bin_to_nat(incr n') + bin_to_nat(n')).
    simpl.    
